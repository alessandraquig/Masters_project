...
class grid_set:
    ...
    def load_grid(self, file):
        print(f"Loading a grid {file}...", end="", flush=True)
...
class Gs2Gs:
    ...
    def rg_vecs(self, x, y, method='linear'):
        ...
        if self.vectors:
            ...
            # interpolate xr values
            if method == 'linear':
                # BREAKING ON THIS LINE - "DIFFERENT NUMBERS OF VALUES AND POINTS"
                interpolator = LinearNDInterpolator(self.tri, xr.ravel())
            elif method == 'nearest':
                interpolator = NearestNDInterpolator(self.tri, xr.ravel())
            elif method == 'cubic':
