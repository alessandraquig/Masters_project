...
for year in range(42):
    for month in range(12):
        conc_avg[year, month] = np.nanmean(conc[year, month, xmin:xmax, ymin:ymax], axis=(0, 1))
...
for year in range(len(years)):
    for month in range(len(months)):
        ...
        tau_a_v_avg[year, month] = np.nanmean(tau[year, month, xmin:xmax, ymin:ymax, 1, 1], axis=(0, 1))
        tau_i_u_avg[year, month] = np.nanmean(tau[year, month, xmin:xmax, ymin:ymax, 0, 2], axis=(0, 1))
...
plt.savefig(f"Maps_output/{hemisphere}/Beaufort_stress_timeseries.png")
